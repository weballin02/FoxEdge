...
def generate_social_media_post(bet):
    ...
    community engagement, dynamic hashtags, and a recent form advantage analysis that aligns with the prediction.
    ...
    # Get recent form values from global stats
    ...
    # Compute recent form advantage based on predicted winner
    if bet['predicted_winner'] == bet['home_team']:
        advantage = round_half(recent_form_home - recent_form_away)
        if advantage > 0:
            form_phrase = f"The home team is on fire with a recent form advantage of {advantage} pts."
        else:
            form_phrase = "Recent form is close, yet our advanced analysis favors the home team."
    else:
        advantage = round_half(recent_form_away - recent_form_home)
        if advantage > 0:
            form_phrase = f"The away team is outscoring opponents recently by {advantage} pts on average."
        else:
            form_phrase = "Despite similar recent form numbers, our analysis favors the away team."
